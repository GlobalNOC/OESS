...
async function getConnectionHistory(id) {
  let url = `[% path %]services/vrf.cgi?method=get_connection_history&connection_id=${id}`;
  try {
    const resp = await fetch(url, {method: 'get', credentials: 'include'});
    const data = await resp.json();
    return data;
  } catch(error) {
    console.log('Failure occurred in getConnectionHistory');
    console.log(error);
  }
}

...
async function getInterfacesByWorkgroup(workgroupID, trunk=1) {
  let url = `services/data.cgi?method=get_workgroup_interfaces&workgroup_id=${workgroupID}&show_down=1&show_trunk=${trunk}`;
...
async function getEntities(workgroupID, parentEntityID=null) {
  ...
  if (parentEntityID !== null) {
      url = `services/entity.cgi?method=get_entity&workgroup_id=${workgroupID}&entity_id=${parentEntityID}`;
  }

...
my $modules = [
    ...
    { name => "provisioning",      source => "../frontend/webservice/provisioning.cgi" },
...
foreach my $module (@$modules) {
    ...
    my $filename = "./_api_endpoints/$module->{name}.md";
    if (! -e $filename) {
        print "Creating initial page for $module->{name}.\n";

        open(FH1, ">_api_endpoints/$module->{name}.md");
        print FH1 "---\n";
    ...
    foreach my $method (@$methods) {
        ...
        open(FH, ">_data/api/$module->{name}/$method.json");
        print FH $1;
    }
}

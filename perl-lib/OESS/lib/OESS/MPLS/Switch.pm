...
sub new{
    ...
    if (!$ok) {
	$self->{'logger'}->error("Oh nooooooooooooooooooooes. Connection to device could not be established.");
    } else {
	$self->{'logger'}->error("Oh yeeeeeeeeeeeeeeeeeeeees. Connection to device was established.");
    }

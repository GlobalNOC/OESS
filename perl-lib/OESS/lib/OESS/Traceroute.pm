...
sub init_circuit_trace {
    ...
    if ($db->get_traceroute_transactions(circuit_id => $circuit_id) ){
        #set_error..
      return;
    }
    ...
    $db->add_traceroute_transaction( circuit_id=> $circuit_id,
                                     source_port => $endpoint_interface,
   ...
   #will have transaction_id,ttl,source_port left of current traceroute
    ...
    $transaction = $db->get_traceroute_transactions(circuit_id => $circuit_id);
    #add all rules
...
sub process_trace_packet {
    ...
    foreach my $circuit (@$circuits){
        my $candidate_transaction = $db->get_traceroute_transactions(circuit_id => $circuit->{'circuit_id'});
        if ($candidate_transaction){
            ...
            if($internal_ids->{$node->{node_name}}{$interface_id}){
    ...
    foreach my $flow_rule ($self->build_trace_rules($circuit_id) {
        ...
        foreach my $endpoint (@{$circuit->{'details'}->{'endpoints'} }){
            ...
            if ($e_interface_id == $src_port ){
                #decrement 
                $db->decrement_traceroute_remaining_endpoints($transaction->{'transaction_id'});
   ...
   $db->decrement_traceroute_ttl(transaction_id => $transaction_id);
   $transaction = $db->get_traceroute_transaction(transaction_id => $transaction_id);
        
                           if ($transaction->{'remaining_endpoints' < 1){
                               #we're done!
            $db->update_transaction(status=>'Complete');
        }
        elsif ($transaction->{'ttl'} < 1){
            $db->update_transaction(status=>'Timeout');
            }
            else {
                $self->send_trace_packet($transaction);
            }

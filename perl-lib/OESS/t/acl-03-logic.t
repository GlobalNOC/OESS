...
sub is_vlan_allowed {
    ...
    foreach my $a (@$acl) {
        my $allow = $a->vlan_allowed(workgroup_id => $args->{workgroup_id}, vlan => $args->{vlan});
        if ($allow == 1) {
            return 1;
        }
        if ($allow == 0) {
...
$raw = OESS::DB::ACL::fetch_all(interface_id => 45901, db => $db );

foreach my $r (@$raw) {
    my $a = OESS::ACL->new(db => $db, model => $r);
    push @$acl, $a;
}
...
$acl = [];
$raw = OESS::DB::ACL::fetch_all(interface_id => 32291, db => $db );
...
va_test(
  is_vlan_allowed( workgroup_id => 31, vlan => 1729 ),
  0,
...
$acl = [];
$raw = OESS::DB::ACL::fetch_all(interface_id => 501, db => $db );
...
va_test(
  is_vlan_allowed( workgroup_id => 11, vlan => 1729 ),
  0,
...
$acl = [];
$raw = OESS::DB::ACL::fetch_all(interface_id => 21, db => $db );

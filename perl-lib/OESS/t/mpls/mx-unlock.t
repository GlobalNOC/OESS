...
my $ok = $device->unlock();
ok($ok == 1, "unlock succeeded when ok message received.");
...
$ok = $device->unlock();
ok($ok == 1, "unlock reported success when warning message received.");
...
$ok = $device->unlock();
ok($ok == 0, "unlock failed when error message received.");


my $err = $mock->sub_called(
    name  => 'close_configuration',
    count => 3
);

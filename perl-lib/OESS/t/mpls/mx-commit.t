...
my $ok = $device->commit();
ok($ok == 1, "commit succeeded when ok message received.");
...
$ok = $device->commit();
ok($ok == 1, "commit reported success when warning message received.");
...
$ok = $device->commit();
ok($ok == 0, "commit failed when error message received.");

...
my $multipoint_circuit_id = 4182;
my $ckt = OESS::Circuit->new( circuit_id => $multipoint_circuit_id, db => $db);

my $expected_flows_1 = get_expected_flows_1();

ok(OESSDatabaseTester::flows_match( 
    actual_flows   => $ckt->get_flows(),
    expected_flows => get_expected_flows_1()
), "Flows are as expected");


sub get_expected_flows_1 {
    ...
    my $obj_flows = [{
       ...
       'match' => {
        ...
        foreach my $flow (@$obj_flows) {
            push(@$flows, OESS::FlowRule->new(%$flow));
        }
